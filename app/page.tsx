...
function Brand({ priority = false }: { priority?: boolean }) {
  return <a className="brand" href="#top"><Image src="/images/logo-kb.png" alt="" width={44} height={44} quality={80} priority={priority} className="brand-logo" /><b>KARACHI<br /><i>BRASSERIE</i></b></a>;
}

function ReelCard({ src, views }: { src: string; views: string }) {
  ...
  return <motion.article className="reel-card" whileHover={{ scale: 1.035 }} transition={{ type: "spring", stiffness: 320, damping: 24 }} onMouseEnter={play} onMouseLeave={stop}><video ref={videoRef} src={src} muted loop playsInline preload="metadata" aria-label={`Karachi Brasserie Instagram reel with ${views} views`} /><motion.div className="reel-overlay" initial={{ opacity: 0 }} whileHover={{ opacity: 1 }} transition={{ duration: .2 }}><RiInstagramLine size={24} /><span>{views} views</span></motion.div></motion.article>;
}

function SocialProof() {
  return <section className="social-proof reveal" aria-labelledby="social-proof-title"><p className="kicker">FROM OUR TABLES</p><h2 id="social-proof-title">Loved by 10,000+ Foodies.</h2><div className="reels-grid"><ReelCard src="/videos/video-1.mp4" views="18.4K" /><ReelCard src="/videos/video-2.mp4" views="24.7K" /><ReelCard src="/videos/video-3.mp4" views="12.9K" /><ReelCard src="/videos/video-4.mp4" views="21.3K" /></div></section>;
}
